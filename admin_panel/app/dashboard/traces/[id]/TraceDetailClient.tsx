    ...
    getTrace(id)
      .then(async (t) => {
        ...
        if (t.parent_trace_id) {
          try {
            const root = await getTrace(t.parent_trace_id)
            if (!cancelled) setRootTrace(root)
          } catch {
            if (!cancelled) setRootTrace(t)
          }
        } else {
          setRootTrace(t)
        }
      })
      .catch(() => { if (!cancelled) router.replace('/dashboard/traces') })
      .finally(() => { if (!cancelled) setLoading(false) })

    return () => { cancelled = true }
  }, [id, router])

  if (loading || !trace || !rootTrace) {
    return (
